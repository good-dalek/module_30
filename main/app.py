...
def create_app():
    ...
    def parking_entrance():
        ...
        opened_parking = (db.session.query(Parking.opened)
                          .where(Parking.id == parking_id)).scalar()
    ...
    def leaving_the_parking():
        ...
        parking_id = data.get('parking_id')
        client_id = data.get('client_id')
        client_parking_id = (db.session.query(ClientParking.id)
                             .filter(ClientParking.client_id == client_id,
                                     ClientParking.parking_id == parking_id)).scalar()

        parking = db.session.query(Parking).get(parking_id)
        credit_card = db.session.query(Client.credit_card).filter(Client.id == data.get('client_id')).scalar()
